#!/usr/bin/env python3
"""Build vanguard_crypto_symbol_state from latest crypto bars/config."""
...
from vanguard.selection.crypto_symbol_state import (  # noqa: E402
    DEFAULT_CONFIG,
    DEFAULT_DB,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build Vanguard crypto symbol-state rows.")
...
def main() -> int:
    ...
    rows = build_crypto_symbol_state(
        db_path=args.db,
        context_config=load_crypto_context_config(args.config),
    ...
    missing = len(rows) - ok
    print(f"crypto_symbol_state rows={len(rows)} ok={ok} missing_or_error={missing} persisted={not args.dry_run}")
