...
class dwx_client():
    ...
    def __init__(self, event_handler=None, metatrader_dir_path='',
        ...
        self._threads = [
            ...
            self.bar_data_thread,
            self.open_orders_thread,
            self.historic_data_thread,
        ]
    ...
    def _load_json_payload(self, text, label='payload'):
        ...
        try:
            ...
            if last_obj is not None:
                if self.verbose:
                    print(f'[DWX] recovered concatenated JSON for {label}')
                return last_obj
            if self.verbose:
                print(f'[DWX] failed to decode {label}')
            return None

    def _latest_orders_path(self):
        candidates = []
        for path in (self.path_orders, self.path_orders_stored):
            if exists(path):
                try:
                    candidates.append((getmtime(path), path))
                except OSError:
                    continue
        if not candidates:
            return None
    ...
    def _read_latest_orders_text(self):
        latest_path = self._latest_orders_path()
        if latest_path is None:
            return ''
        return self.try_read_file(latest_path)
    ...
    def check_open_orders(self):
        ...
        while self.ACTIVE:
            ...
            text = self._read_latest_orders_text()

            if len(text.strip()) == 0 or text == self._last_open_orders_str:
                continue

            data = self._load_json_payload(text, 'orders')
            self._last_open_orders_str = text
            if not isinstance(data, dict) or 'orders' not in data or 'account_info' not in data:
                continue
    ...
    def check_messages(self):
        ...
        while self.ACTIVE:
            ...
            data = self._load_json_payload(text, 'messages')
    ...
    def check_market_data(self):
        ...
        while self.ACTIVE:
            ...
            data = self._load_json_payload(text, 'market_data')
    ...
    def check_bar_data(self):
        ...
        while self.ACTIVE:
            ...
            data = self._load_json_payload(text, 'bar_data')
    ...
    def check_historic_data(self):
        ...
        while self.ACTIVE:
            ...
            if len(text.strip()) > 0 and text != self._last_historic_data_str:
                ...
                data = self._load_json_payload(text, 'historic_data')
            ...
            if len(text.strip()) > 0 and text != self._last_historic_trades_str:
                ...
                data = self._load_json_payload(text, 'historic_trades')
    ...
    def load_orders(self):
        ...
        if len(text) > 0:
            data = self._load_json_payload(text, 'orders_bootstrap')
            self._last_open_orders_str = text
            if not isinstance(data, dict):
                return
            self.account_info = data['account_info']
    ...
    def load_messages(self):
        ...
        if len(text) > 0:
            ...
            data = self._load_json_payload(text, 'messages_bootstrap')
            self._last_messages_str = text
            if not isinstance(data, dict):
                return
